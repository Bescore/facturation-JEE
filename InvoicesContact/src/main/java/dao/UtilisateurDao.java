...
public class UtilisateurDao implements Interface<Utilisateur> {
	...
	public boolean Create(Utilisateur object) {
		...
		try {
			PreparedStatement sql = connect
					.prepareStatement("INSERT INTO utilisateur(nom,prenom,email,password,telephone) VALUES(?,?,?,?,?)");
	...
	public boolean Update(Utilisateur object) {
		...
		try {
			PreparedStatement sql = connect.prepareStatement(
					"UPDATE  utilisateur SET nom=?,prenom=?,email=?,telephone=?,adresse=?,code_postale=? WHERE id_utilisateur=?");
	...
	public ArrayList<Utilisateur> Findby(Utilisateur object) {
		...
		try {
			PreparedStatement sql = connect
					.prepareStatement("SELECT * FROM utilisateur where email=? AND isActive_personne=?");
